...
class NN(nn.Module):
    def __init__(self, n_nodes, n_particles, hidden_layers, activation):
        super().__init__()
    ...
    def forward(self, x):
        idx = np.argmax(self.nn(x), axis=1)
        return (np.repeat(idx, 3) + np.tile([-1, 0, 1], self.n_particles)) % self.n_nodes
...
class RomSimulation:
    def __init__(self,
        ...
        self.model = NN(self.n_nodes, self.n_particles, hidden_layers, activation)
    ...
    def interpolate_particles_to_field(self):
        px = self.psi_px @ self.px
        with torch.no_grad():
            node_idx = self.model(torch.from_numpy(px.reshape(-1, 1)).float())
        self.interpolation = sparse.coo_matrix((spline(self.get_distance(px[self.pidx], self.node_positions[node_idx])/self.dx, 1), (node_idx, self.pidx)), shape=(self.n_nodes, self.n_particles))
        self.nq = const.q_electron*self.weight_factor*self.interpolation.sum(axis=1).A.ravel() + self.background_charge_density*self.dx
    ...
    def run(self, n_particle_modes, pod_type: Literal['POD', 'PSD', 'Full'] = 'POD', save_snapshots=True):
        ...
        self.pidx = np.repeat(np.arange(self.n_particles), 3)
