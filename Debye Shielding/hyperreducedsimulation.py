...
class PodSimulation:
    ...
    def interpolate_particles_to_field(self):
        x_ref = self.get_distance(self.psi_p @ self.px, self.node_positions.reshape(-1, 1))/self.dx
        self.interpolation = spline(x_ref, self.particle_order)
        self.nq = const.q_electron*self.weight_factor*np.sum(self.interpolation, axis=1) + self.background_charge_density(self.node_positions)*self.dx

    def update_electric_field(self):
        self.ne_field = self.electric_field_matrix @ self.nq

    def interpolate_field_to_particles(self):
        reduced_interpolation = self.interpolation.T[self.particle_measurement_idx]
        reduced_pe_field = reduced_interpolation @ self.ne_field
        self.pe_field = self.unhyperreduce_pe_field @ reduced_pe_field
    ...
    def run(self, n_particle_modes, percent_hyperreduction_points: float=100,
        ...
        match(type):
            case 'PSD':
                ...
                u_pe_field = romtools.get_pod_basis(self.pe_field_snapshots)
                psi_pe_field = u_pe_field[:, :n_particle_modes]

                self.particle_measurement_idx = construct_measurments(self.n_particles, percent_hyperreduction_points, hyperreduction_algorithm, u_pe_field)
                self.unhyperreduce_pe_field = self.psi_p.T @ psi_pe_field @ np.linalg.pinv(psi_pe_field[self.particle_measurement_idx])
