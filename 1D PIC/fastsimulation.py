...
class Simulation:
    def __init__(self,
        ...
        self.px, self.pv = PdfSampler.sample(f0, [x_domain, v_domain], n_particles_per_cell*n_nodes)
    ...
    def interpolate_particles_to_field(self):
        x_ref = self.get_distance(self.px, self.node_positions.reshape(-1, 1))/self.dx
    ...
    def save_snapshot(self):
        self.snapshots.append(Snapshot(self.time, self.px.copy(), self.pv.copy(), self.pe_field.copy(), self.nq.copy(), self.ne_field.copy(), self.interpolation.copy()))
    ...
    def show_snapshots(self, fps=10, save_animation=False, filename='PIC_simulation', repeat=True, show_moments=True, show_cells=False):
        ...
        if show_moments:
            moment_array = np.array([get_moments(self.shift_x_to_domain(s.x), s.v, self.node_positions, self.weight_factor) for s in self.snapshots])
    ...
    def save_snapshots_to_csv(self, filename):
        ...
        for s in self.snapshots:
            ...
            interpolation_list.append(s.interpolation.ravel())
