...
if __name__ == "__main__":
    ...
    U, svd, _ = np.linalg.svd(nq, full_matrices=False, compute_uv=True)
    # plt.plot(np.cumsum(svd) / np.sum(svd))
    # plt.show()
    n_node_modes = 40
    psi_q = U[:, :n_node_modes]
    print(romtools.proj_error(psi_q, nq))

    rq = psi_q.T @ nq

    nrq, meanq, stdq = nntools.normalize(rq)
    npe, meane, stde = nntools.normalize(pe)

    x = torch.from_numpy(nrq.T)
    y = torch.from_numpy(npe.T)

    input_size = n_node_modes
    output_size = n_particles
    ...
    torch.save(network.state_dict(), f'{filename}_romq_to_pe_net')
    np.savetxt(f'{filename}_romq_to_pe_scaling', np.array([meanq, stdq, meane, stde]), header='meanq, stdq, meane, stde')
