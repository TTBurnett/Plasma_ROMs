...
class PodSimulation:
    ...
    def accelerate_particles(self):
        x_ref = self.get_distance(self.psi_p[self.particle_idx] @ self.px, self.node_positions[self.node_idx])/self.dx
        reduced_interpolator = spline(x_ref, self.particle_order)
        pe_field = const.q_electron * self.weight_factor * np.einsum('a,abc,c', reduced_interpolator, self.operator, reduced_interpolator, optimize=True)
    ...
    def run(self, n_particle_modes, percent_hyperreduction_points: float=100, percent_hyperreduction_modes: float=100,
        ...
        match(type):
            case 'PSD':
                ...
                psi_interpolations, s, _ = randomized_svd(self.interpolation_snapshots, round(1e-2*self.n_particles*self.n_nodes*percent_hyperreduction_modes))

                print('Determining measurement locations...')
                interpolation_measurement_idx = construct_measurments(self.n_particles*self.n_nodes, percent_hyperreduction_points, hyperreduction_algorithm, psi_interpolations)
                self.particle_idx = interpolation_measurement_idx % self.n_particles
                self.node_idx = interpolation_measurement_idx // self.n_particles
                unhyperreduce_interpolation = (psi_interpolations @ np.linalg.pinv(psi_interpolations[interpolation_measurement_idx])).reshape(self.n_nodes, self.n_particles, -1)
        ...
        # Create tensor for online portion
        print('Creating tensor for online calculations...')
        self.operator = np.einsum('ab,cdb,de,efg->cag', self.psi_p.T, unhyperreduce_interpolation.transpose(2, 0, 1),
                                  electric_field_matrix, unhyperreduce_interpolation, optimize=True)
        print(self.operator.shape)
