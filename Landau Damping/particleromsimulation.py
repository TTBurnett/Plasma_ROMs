...
class Snapshot:
    def __init__(self, time, px, pv, nrho, e_field):
        self.time = time
        self.x = px
        self.v = pv
        self.nrho = nrho
...
class RomSimulation:
    ...
    def save_snapshot(self):
        self.snapshots.append(Snapshot(self.time, self.shift_x_to_domain(self.psi_px @ self.px), self.psi_pv @ self.pv, self.nrho.copy(), self.ne_field.copy()))
    ...
    def compare_electric_field(self, node_snapshots, colors=['m', 'c']):
        print('Generating plot...')
        e_field_rom = np.abs(np.array([fft.fft(s.e_field)[1] for s in self.snapshots]))
        e_field_fom = np.abs(np.array([fft.fft(node_snapshots[:, i])[1] for i in range(node_snapshots.shape[1])]))
        e_fields = [e_field_fom, e_field_rom]
        labels = ['FOM', 'ROM']
        
        fig = plt.figure(figsize=(10, 5))
        for e_field, label, color in zip(e_fields, labels, colors):
            time = np.array([s.time for s in self.snapshots])
            plt.plot(time, e_field, label=f'{label}', color=color)
            ...
            plt.plot(time[decay_idx], decay_line, color=color, linestyle='--', label=f'{label}: $\gamma$ = {gamma_decay:.3g}')
            plt.plot(time[growth_idx], growth_line, color=color, linestyle=':', label=f'{label}: $\gamma$ = {gamma_growth:.3g}')
        plt.legend()
        plt.show()